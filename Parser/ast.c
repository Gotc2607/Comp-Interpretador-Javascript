...
static int ast_check_node(ASTNode *node)
{
    ...
    case AST_FOR:
        ...
        if (node->left)
            ok &= ast_check_node(node->left);
        if (node->mid)
            ok &= ast_check_node(node->mid);
        if (node->extra)
            ok &= ast_check_node(node->extra);
        ok &= ast_check_node(node->right);
        in_loop_depth--;
        return ok;
    ...
    case AST_BREAK:
        if (in_loop_depth == 0)
        {
            semantic_error_loc(node->line, node->col,
    ...
    case AST_DECLARE:
        sem_declare(node->text, node->line, node->col);
        ok &= ast_check_node(node->left);
        return ok;
    ...
    case AST_FUNC_CALL:
        ...
        if (!sem_exists(node->text))
        {
            semantic_error_loc(node->line, node->col, "Erro de referencia: funcao '%s' chamada sem ser declarada.", node->text);
            ok = 0;
        }
        {
            ASTNode *a = node->left;
