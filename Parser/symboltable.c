...
void scope_pop(void) {
    if (!current->parent) {
        fprintf(stderr, "Erro interno: tentou sair do escopo global.\n");
        return;
    }
    for (int i = 0; i < HASH_SIZE; i++) {
        ...
        while (s) {
            Symbol *next = s->next;
            if (s->type == SYM_STRING && s->sval) free(s->sval);
            free(s);
            s = next;
        }
    }
...
void sym_declare(const char *name, int is_const) {
    ...
    if (s) {
        if (s->is_const != 0) {
            fprintf(stderr,
                "Erro de Sintaxe: Identificador '%s' ja foi declarado neste escopo.\n",
                name);
        }
        return;
    }
...
void sym_set_int(const char *name, int value) {
    ...
    if (s->is_const == 1) {
        fprintf(stderr,
            "TypeError: Atribuicao a variavel constante '%s'.\n",
            name);
        return;
    }
    if (s->is_const == 2) s->is_const = 1;
    s->type = SYM_INT;
    s->ival = value;
    s->initialized = 1;
}
...
int sym_get_int(const char *name) {
    Symbol *s = find_symbol(name);
    if (s && s->type == SYM_INT) return s->ival;
    return 0;
}
...
char *sym_get_str(const char *name) {
    Symbol *s = find_symbol(name);
    if (s && s->type == SYM_STRING) return s->sval;
    return "";
}
...
ASTNode *sym_get_func(const char *name) {
    Symbol *s = find_symbol(name);
    if (s && s->type == SYM_FUNCTION) return s->func_body;
    return NULL;
}
...
void sym_set_array_element(const char *name, int index, int value) {
    if (index < 0 || index > 1000000) {
        fprintf(stderr,
            "Erro de Seguranca: Indice %d invalido para o array '%s'.\n",
            index, name);
        return;
    }
    Symbol *s = find_symbol(name);
    ...
    if (index >= s->arr_size) {
        ...
        if (!s->arr_vals) {
            fprintf(stderr,
                "Erro de Seguranca: Memoria insuficiente para array '%s' (tamanho %d).\n",
                name, novo_tamanho);
            exit(EXIT_FAILURE);
        }
        for (int i = s->arr_size; i < novo_tamanho; i++) s->arr_vals[i] = 0;
